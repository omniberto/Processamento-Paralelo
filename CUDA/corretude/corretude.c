...
int main(int argc, char** argv) {
    ...
    for (unsigned long int i = 0; i < total_pixels; i++) {
        // Calcula a diferenca absoluta entre o pixel do Serial e do CUDA
        int diff = abs((int)img_serial[i] - (int)img_cuda[i]);
    ...
    if (pixels_divergentes == 0) {
        ...
        if (diferenca_maxima == 1) {
            ...
            printf("\nVEREDITO: FALHA.\n");
            printf("As imagens sao visivelmente diferentes. Existe algum erro de logica (provavelmente nas Ghost Cells) no codigo cuda.\n");
        }
    }
    printf("==================================================\n");
